...
from New_version_Unet.find_contours import get_region_to_annotate
from tissue_batch_processing import image_from_WSI
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        x0, y0 = int(self.wsi.properties['openslide.bounds-x']), int(self.wsi.properties['openslide.bounds-y'])
        h = int(int(self.wsi.properties['openslide.bounds-height'])/(2**seg_level))
        w = int(int(self.wsi.properties['openslide.bounds-width'])/(2**seg_level))

        # Read image
        img = np.array(self.wsi.read_region((x0, y0), seg_level, (w, h)))  
        # Crear una imagen de Pillow a partir del array

        if self.name == 'AR23B002607A001003':
            image = Image.fromarray(img)

            # Guardar la imagen como PNG
            image.save('imagen_original.png')

        # Mask no elements (alpha channel 0) as background (white)
        img[img[:,:,3] == 0] = [255, 255, 255, 255]
        img = img[:,:,:3]
        geojson_path = os.path.join('/mnt/work/users/valeria.caro/WSI/WSI/geojson', self.name +'.geojson')
        region_to_annotate_coords = get_region_to_annotate(geojson_path)
        # image, W, H, _ = image_from_WSI('/home/usuaris/imatge/valeria.caro/TILES/WSI/' + self.name + '.mrxs')
        # Calculate factor based on image dimensions
        # factor = int(np.round(H / img.shape[0]))
        region_to_annotate_coords_low_resolution = region_to_annotate_coords // 2**seg_level
        if region_to_annotate_coords_low_resolution is not None:
            mask = np.zeros_like(img)
            mask = cv2.fillPoly(mask, pts=np.int32([region_to_annotate_coords_low_resolution]), color=(255,255,255))
            mask = mask // 255
            img = img * mask
            img[img==0] = 1
        ...
        # Convert contour coordinates to GeoJSON format
        features = []
        for idx, coordinates in enumerate(contour_coordinates):
            # Create GeoJSON feature for each contour
            coordinates.append(coordinates[0])
            feature = {
                "type": "Feature",
                "geometry": {
                    "type": "Polygon",
                    "coordinates": [coordinates]  # Assuming each contour is represented as a single polygon
                },
                "properties": {
                    "index": idx
                }
        ...
        output_path = "/mnt/work/users/valeria.caro/WSI/WSI/new_geojson/" + self.name + '.geojson'
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        # output_path = '/home/usuaris/imatge/valeria.caro/TILES/WSI/new_geojsons/'
        # geojson_path = os.path.join('/home/usuaris/imatge/valeria.caro/TILES/WSI/geojson/', self.name +'.geojson')
        # region_to_annotate_coords = get_region_to_annotate(geojson_path)
        # image, W, H, _ = image_from_WSI('/home/usuaris/imatge/valeria.caro/TILES/WSI/' + self.name + '.mrxs')
        # factor = int(np.round(H / img.shape[1]))
        # contours = png2geojson(img, factor)
        # save_geojson(contours, self.name, output_path)
